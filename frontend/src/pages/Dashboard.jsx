...
export default function Dashboard() {
  ...
  const fetchDecks = async () => {
    try {
      const res = await fetch(`http://localhost:5000/api/decks?sessionId=${getSessionId()}`);
  ...
  const handleDelete = async (e, id) => {
    ...
    try {
      await fetch(`http://localhost:5000/api/decks/${id}?sessionId=${getSessionId()}`, { method: 'DELETE' });
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {decks.map((deck, index) => {
              ...
              return (
                <Link key={deck._id} to={`/deck/${deck._id}`} className="group relative bg-white rounded-none p-6 border-4 border-black shadow-[8px_8px_0px_0px_black] hover:-translate-y-2 hover:shadow-[12px_12px_0px_0px_black] transition-all duration-200 ease-out">
